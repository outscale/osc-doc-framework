...
function generateOapiCliPartials (apiMarkdown, api, outputFolder) {
  ...
  createOapiCliSections(api, codeSamples, outputFolder)
}
...
function createCodeSamples (apiMarkdown) {
  ...
  for (let i = 0, length = matches.length; i < length; i++) {
    ...
    if (k.endsWith('.sh')) {
      v = v.replaceAll('oapi-cli', '$ oapi-cli')
      v = v.replaceAll('\n  ', '\n    ')
    }
    codeSamples[k] = v
  }
...
function createOapiCliSections (api, codeSamples, outputFolder) {
  ...
  for (const path of Object.values(paths)) {
    ...
    s += formatMainDescription(description, operation)
    s += '\n'
    ...
    fs.writeFileSync(process.cwd() + '/' + outputFolder + '/_RC-OapiCli-' + operation + '.adoc', s)
  }
}
...
module.exports = generateOapiCliPartials
