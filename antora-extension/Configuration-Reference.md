...
* `build_options`:
    ...
    * `octl_prefix`: (optional) The service prefix corresponding to this API in `octl`. For example, `iaas` for OUTSCAPE API or `kube` for OKS API.
    * `octl_partials`: (optional) If `true`, Antora partials are generated for the octl User Guide sections of this API (`shell--octl` must be among the specified `languages`, and you must also specify `octl_prefix`).
